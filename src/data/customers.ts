...
export const CUSTOMER_STORIES = [
  ...
  },
  {
    ...
    industry: '금융' as const,
    slug: 'dongyang-life',
    summary: '노후화된 메타데이터 솔루션 교체 및 모델링 솔루션 도입으로 데이터 관리체계 자동화 구현',
    detail: {
      companyDesc: '1989년 동양베네피트생명으로 설립된 생명보험 전문기업',
      pageHeading: '동양생명, 데이터 관리체계 자동화 솔루션 도입',
      background: [
        '노후화된 메타데이터 솔루션 교체 및 모델링 솔루션 도입',
        '모델 변경사항을 CSR 시스템으로 DB에 연동하여 반영',
        '모델 현행화를 위한 표준 갭 분석',
      ],
      features: ['DA#, META# 기반 데이터 관리 자동화'],
      effects: [
        '데이터 품질관리를 통한 고품질 데이터 확보',
        '소통 원활화 및 운영 효율성 개선',
        '데이터 기반 비즈니스 의사결정 지원',
        '전사 IT 자산 관리 환경 제공',
      ],
      quote: '메타데이터 관리는 대용량 데이터 중 원하는 정보를 효율적으로 찾아낼 수 있도록 다른 데이터를 설명해 주는 데이터를 관리하는 것이다. 이를 통해 기업의 데이터 자산을 체계적으로 관리하고 활용할 수 있다.',
      meta: {
        date: '2021-11-10',
        industry: '금융',
        purpose: '데이터 관리체계 자동화, 고품질 데이터 확보',
      },
    } as CustomerDetail,
  },
];
