...
const DEFAULT_HERO = { title: '다운로드 신청', desc: 'DA# 무료 다운로드 및 제품 소개서를 신청하세요.' };

export default function DownloadPageClient({ ssrContent }: { ssrContent: Record<string, string> }) {
  ...
  const [hero, setHero] = useState(() => safeParse(ssrContent.download_hero, DEFAULT_HERO));

  useEffect(() => {
    if (!editMode) return;
    const handler = (e: MessageEvent) => {
      if (e.data?.type === 'content-update' && e.data.section === 'download_hero') {
        setHero(e.data.data);
      }
    };
      ...
      >
        ...
        <div ref={heroRef} style={{ padding: '0 56px', position: 'relative', zIndex: 1 }}>
          ...
          <h1 data-hero style={{ fontSize: 'clamp(40px, 5vw, 64px)', fontWeight: 800, color: '#F9FAFB', letterSpacing: '-0.04em', lineHeight: 0.95, marginBottom: '16px' }}>
            <E id="download_hero.title" editMode={editMode}>{hero.title}</E><span style={{ color: '#36c88a', fontSize: '1.1em' }}>.</span>
          </h1>
          <p data-hero style={{ fontSize: '16px', color: 'rgba(255,255,255,0.4)', lineHeight: 1.7, maxWidth: '480px' }}><E id="download_hero.desc" editMode={editMode}>{hero.desc}</E></p>
        </div>
