        ...
        {CATEGORIES.map((cat) => (
          <button key={cat.key} onClick={() => setActiveCategory(cat.key)}
            ...
            style={activeCategory === cat.key
              ? { color: '#36c88a', borderBottom: '2px solid #36c88a', marginBottom: '-1px', backgroundColor: 'transparent' }
              : { color: '#888d94', borderBottom: '2px solid transparent', marginBottom: '-1px', backgroundColor: 'transparent' }
            }
          >
            {cat.label}
          </button>
      ...
      <div className="space-y-4">
        {filtered.map((post) => (
          <div key={post.id}
            ...
            style={{ backgroundColor: '#ffffff', border: '1px solid #e6e8ec', borderRadius: '0', boxShadow: '0 4px 20px rgba(24,39,33,.07)' }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = '#36c88a'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 20px rgba(54,200,138,.12)'; }}
            onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = '#e6e8ec'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 20px rgba(24,39,33,.07)'; }}
          >
            <div className="flex justify-between items-start">
              <div>
                <span className="text-xs px-2 py-1 rounded font-medium" style={{ backgroundColor: '#e9f9f1', color: '#36c88a' }}>{post.category}</span>
