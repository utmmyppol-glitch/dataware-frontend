...
NETWORK="union-net"
ALIAS="dataware-frontend-active"
IMAGE="${DOCKER_IMAGE:?DOCKER_IMAGE not set}"

# 네트워크 없으면 최초 1회 생성
docker network create "$NETWORK" >/dev/null 2>&1 || true

if docker ps --format '{{.Names}}' | grep -qx dataware-frontend-blue; then
  CURRENT=dataware-frontend-blue
  NEW=dataware-frontend-green
else
  CURRENT=dataware-frontend-green
  NEW=dataware-frontend-blue
fi
...
docker run -d --name "$NEW" --network "$NETWORK" --restart unless-stopped \
  -e BACKEND_URL="${BACKEND_URL:-http://union-backend-active:8080}" \
  "$IMAGE"
...
i=0
while [ "$i" -lt 15 ]; do
  if docker run --rm --network "$NETWORK" curlimages/curl:8.10.1 -sf "http://$NEW:3001/" >/dev/null 2>&1; then
    OK=1
    break
  fi
  i=$((i + 1))
